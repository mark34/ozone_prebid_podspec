Pod::Spec.new do |s|
  ...
  s.name         = "PrebidMobile"
  s.version      = "2.1.2"
  s.summary      = "mark - PrebidMobile is a lightweight framework that integrates directly with Prebid Server."

  s.description  = <<-DESC
    mark - Prebid-Mobile-SDK is a lightweight framework that integrates directly with Prebid Server to increase yield for publishers by adding more mobile buyers."
    DESC
...
# NOTE that our tags have to be called ozone-[M.m.b] because the [M.m.b] names are already taken by prebid upstream repo
  s.source      	   = { :git => "https://github.com/mark34/ozone-prebid-mobile-ios.git", :tag => "ozone-#{s.version}" }
#  s.source      	   = { :git => "https://github.com/mark34/ozone_prebid_framework.git", :tag => "#{s.version}" }
#  s.source      	   = { :git => "https://github.com/prebid/prebid-mobile-ios.git", :tag => "2.0.8" }
